...
void two_byte(int len){
    ...
    uint16_t** mat_a = (uint16_t**) malloc(len*sizeof(uint16_t*));
    uint16_t** mat_b = (uint16_t**) malloc(len*sizeof(uint16_t*));
    uint16_t** mat_o = (uint16_t**) malloc(len*sizeof(uint16_t*)); //height from a

    for(int m_row = 0; m_row<len; m_row++){
        mat_a[m_row] = (uint16_t*) aligned_alloc(32,len*sizeof(uint16_t));
        mat_o[m_row] = (uint16_t*) aligned_alloc(32,len*sizeof(uint16_t)); //width from b
    }

    for(int m_row = 0; m_row<len; m_row++){
        mat_b[m_row] = (uint16_t*) aligned_alloc(32,len*sizeof(uint16_t));
    }
    ...
    // init the matrix to store the naive solution
    uint16_t** mul = (uint16_t**) malloc(len*sizeof(uint16_t*));
    for(int m_row = 0; m_row<len; m_row++){
        mul[m_row] = (uint16_t*) aligned_alloc(32,len*sizeof(uint16_t));
        for(int i = 0; i<len;i++){
            mul[m_row][i]=0;
        }
    }
    ...
    for(int i=0;i<len;i++){
        for(int j=0;j<len;j++){
            // if it is not within an acceptable range, output it
