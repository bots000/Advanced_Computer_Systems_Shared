...
void four_byte(int len){
    ...
    float** mat_a = (float**) malloc((len+leftover)*sizeof(float*));
    float** mat_b = (float**) malloc((len+leftover)*sizeof(float*));
    float** mat_o = (float**) malloc((len+leftover)*sizeof(float*));

    // used for naive checking at the end
    float** mul = (float**) malloc((len+leftover)*sizeof(float*));

    for(int m_row = 0; m_row<(len+leftover); m_row++){
        mat_a[m_row] = (float*) aligned_alloc(32,(len+leftover)*sizeof(float));
        mat_o[m_row] = (float*) aligned_alloc(32,(len+leftover)*sizeof(float));
        mat_b[m_row] = (float*) aligned_alloc(32,(len+leftover)*sizeof(float));
        mul[m_row] = (float*) aligned_alloc(32,(len+leftover)*sizeof(float));
    }
