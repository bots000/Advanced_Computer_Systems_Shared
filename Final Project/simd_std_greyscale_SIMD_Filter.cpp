...
//J0 -> yyyyyy
static void Rgb2GraySingleRow_useSSE(const unsigned char I0[], const int image_width, unsigned char J0[])
{
    int x;      //Index in J0.
...
void Rgb2Gray_useSSE(unsigned char I[], int image_width,  int image_height, unsigned char J[]){
    ...
    for (y = 0; y < image_height; y ++)
    {
        ...
        Rgb2GraySingleRow_useSSE(I0, image_width, J0);
    }
...
int main() {
    ...
    Rgb2Gray_useSSE(rgb_image, width, height, grey_img);
    ...
    maskHiLo(rgb_image, filt_img, 255, 255, 3, width*height);
    maskHiLo(rgb_image+1, filt_img+1, 0, 255, 3, width*height);
    maskHiLo(rgb_image+2, filt_img+2, 0, 255, 3, width*height);

    stbi_write_png("try_filter_greyscale.png", width, height, 3, filt_img, width);
