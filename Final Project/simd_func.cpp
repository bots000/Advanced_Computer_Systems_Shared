...
void maskHiLoRGB2(unsigned char* dataIn, unsigned char* dataOut, char r_lo, char r_hi, char g_lo, char g_hi, char b_lo, char b_hi, int pixels){
    ...
    for(int i = 0; i*32 < new_pixels; i++){
        ...
        memOffset = i*96;
        //printf("A");
        R = _mm256_load_si256((__m256i*) &dataIn[memOffset]); //32 bytes, mixed, starting at R
        //_mm256_storeu_si256((__m256i*) tdataR, R);
        //printf("B");
        B = _mm256_load_si256((__m256i*) &dataIn[memOffset+32]); //32 bytes, mixed, starting at B
        //_mm256_storeu_si256((__m256i*) tdataB, B);

        //printf("C\n");
        //printf("\nMemOffset: %d\n",memOffset+64);

        G = _mm256_load_si256((__m256i*) &dataIn[memOffset+64]); //32 bytes, mixed, starting at G
        //_mm256_storeu_si256((__m256i*) tdataG, G);
        ...
        //_mm256_storeu_si256((__m256i*) tdataR, t1);
        //_mm256_storeu_si256((__m256i*) tdataG, t2);
        //_mm256_storeu_si256((__m256i*) tdataB, t3);
        ...
        _mm256_storeu_si256((__m256i*) tdataG, t1);
        _mm256_storeu_si256((__m256i*) tdataB, t2);
...
int main(){
    ...
    unsigned char* tdataIn = (unsigned char*) aligned_alloc(32,3*32*sizeof(char));
    unsigned char* tdataOut = (unsigned char*) aligned_alloc(32,3*32*sizeof(char));
    for(int i = 0; i< 96; i++){
        tdataIn[i] = i/3;
    }
    maskHiLoRGB2(tdataIn,tdataOut,0,255,0,255,0,255,32);
